...
Notes: python exif_rename_files.py Input ~/Downloads/folder --output-directory ~/Downloads/folder/x -N
...
def get_command_line():
    ...
    if options.CopyTree and options.OutputDirectory is None:
        print(
            "Error: option '--copy-recursive-tree' should be used with '--output-directory'. "
            "Please provide an output directory or do not use this option. Exiting."
        )
        exit(2)
    ...
    if options.OutputDirectory is not None and not os.path.isdir(
        ...
        print(
            "Error: Directory '%s' provided in '--output-directory' does not exist or is not a directory. "
            "Please provide a valid output directory. Exiting."
            % (options.OutputDirectory)
        )
        exit(3)
